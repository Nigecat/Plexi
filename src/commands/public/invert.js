...
module.exports = {
    args: [],
    perms: [],
    description: "Invert the colours in the previous message (as long as it is an image)",
            ...
            try {
                ...
                if ((image.attachments.size > 0 && image.attachments.every(attachIsImage)) || image.embeds.length > 0) {
                    ...
                    Jimp.read(url).then(image => {
                        image.invert()
                            .write(`./commands/resources/temp/invert.png`)
                        message.channel.send({ files: [ "./commands/resources/temp/invert.png" ] });
                    });
                } else {
