...
export class DataStore extends EventEmitter {
    ...
    async connect() {
        this.emit("debug", `Connecting to database ${this.filename} with namespace: ${this.namespace}`);
    ...
    async set(id: string, value: string) {
        this.emit("debug", `Setting value for ${this.namespace}${id} to ${value}`);
    ...
    async get(id: string): Promise<string> {
        this.emit("debug", `Retrieving value for ${this.namespace}:${id}`);
    ...
    async delete(id: string) {
        this.emit("debug", `Deleting value for ${this.namespace}:${id}`);
